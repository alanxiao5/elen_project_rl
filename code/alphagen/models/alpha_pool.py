...
class AlphaPool(AlphaPoolBase):
    ...
    def try_new_expr(self, expr: Expression, timesteps) -> float:
        ic_ret, ic_mut = self._calc_ics(expr, ic_mut_threshold=0.90)
        ...
        #if self.reward_shaping == 'IC':
        new_ic_ret = self.evaluate_ensemble()
        
        if self.reward_shaping == 'IR':
            new_reward = self.evaluate_deflator()
            LAMBDA = 0.02
            MAX_IR = 0.5
            SLOPE = 4.5e-6
            TIME_DELAY = 5e4
            clip_reward = np.clip((timesteps-TIME_DELAY)*SLOPE,0,MAX_IR)
            new_ic_ret = new_ic_ret - (new_reward <= clip_reward)*LAMBDA
    ...
    def evaluate_deflator(self) -> float:
        if self.reward_shaping == 'IR': 
            rw = self.calculator.calc_pool_IR_ret(self.exprs[:self.size], self.weights[:self.size])
        elif self.reward_shaping == 'Sortino': 
            rw = self.calculator.calc_pool_SOR_ret(self.exprs[:self.size], self.weights[:self.size])
        elif self.reward_shaping == 'Q10':
            rw = self.calculator.calc_pool_q10_ret(self.exprs[:self.size], self.weights[:self.size])
        elif self.reward_shaping == 'M10':
            rw = self.calculator.calc_pool_m10_ret(self.exprs[:self.size], self.weights[:self.size])    
        return rw    
